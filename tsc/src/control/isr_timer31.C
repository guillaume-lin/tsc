...
extern unsigned char lamp_input_count[12][8];
...
unsigned char data i,j,k,ctemp;
unsigned char xdata * data pp;
		...
		switch (AMX0SL){
			...
			default: {
				if (AMX0SL > 0x08) {
					...
					if (f_key2_int) f_manual_use = 1;		// 手动/自动选择开关闭合
					else f_manual_use = 0;					// 手动/自动选择开关打开
					f_key2_int = 0;
	...
	if (((SYStime_count % 6) == 0x00) & !f_rd_lamp_input_now) {		// 定时3ms 读入灯状态（保存最近 7 次读入值）
		j = 0x00;
		pp = ADDRESS_74HC244K;
		for (i=0x00;i<(MAX_LAMP_NUM - 1)/12 + 1;i++) {
			...
			ctemp = 0x80;
			for (k=0x00;k<8;k++) {
				if ((lamp_input_buffer[ii][j] & ctemp) == 0x00) lamp_input_count[j][k]++;	
				ctemp = ctemp >> 1;
				}
