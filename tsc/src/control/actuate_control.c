...
////061010		if(is_term_connect_to_green(i+1) && 
////061010				decode_term_status(p, i+1) == ON){
////061010			// 有绿灯亮,判断为相位步
////061010			return 1;
////061010		};
////061010	端子 9 18 27 36 可固定常绿，对感应控制没影响
////	if(is_term_connect_to_green(i+1) && (decode_term_status(p, i+1) == ON) && ((i+1)%9!=0x00)) {
//		if(p->m_phase[i])
//		{
//			// 有绿灯亮,判断为相位步
//			return 1;
...
int simple_full_actuate_control(int mode_switch, int timer);
int simple_half_actuate_control(int mode_switch, int timer);
int actuate_control(int mode_switch,int timer)
{
	//static int is_switching;
	//if(mode_switch){
	//	if (!f_return_actuate_control) is_switching = 1;		// 由手动返回则不经过黄闪、全红阶段
	//	else f_return_actuate_control = 0;
	//}
	//
	//if(is_switching){
	//	int ret = switching_with_yellow_flash_all_red(mode_switch,5,5,timer);
	//	if(ret == 1){
	//		is_switching = 0;	// switching done
	//		mode_switch = 1;
	//	}
	//	else
	//		return 0;
	...
	switch(g_config.m_actuate_control_param.m_strategy){
		case STRATEGY_FULL_ACTUATE:											// 2
			// 全感应控制
			return simple_full_actuate_control(mode_switch,timer);
			break;
		case STRATEGY_HALF_ACTUATE:											// 1
			// 半感应控制
			return simple_half_actuate_control(mode_switch,timer);
			break;
		default:
			break;
	}
...
int simple_full_actuate_control(int mode_switch, int timer)
{
	...
	if(timer == 0&&is_step_end())
	{
		...
		else if(is_last_stage_first_step())
		{
			byte s,p,t;
			// 根据需要运行新的时段
			byte times;
